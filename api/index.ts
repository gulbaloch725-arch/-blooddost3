...
// Middleware to ensure DB is ready or show error
app.use(async (req: any, res: any, next: any) => {
  ...
  if (dbInitializationError) {
    return res.status(500).json({ 
      ...
      message: dbInitializationError 
    });
  }
  
  if (!isDbInitialized) {
    // If we are on Vercel or in production, we might need to wait a bit
    if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
      let retries = 5;
      while (!isDbInitialized && retries > 0) {
        console.log(`[SERVER] Waiting for DB initialization for ${req.path}... (${retries} retries left)`);
        await new Promise(r => setTimeout(r, 1000));
        retries--;
      }
      
      if (!isDbInitialized) {
        return res.status(503).json({ error: 'System initializing, please try again in a few seconds' });
      }
    }
  }
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    const allUsers = await db.prepare('SELECT email, role FROM users').all() as any[];
    res.json({ 
      status: "ok", 
      ...
      isPostgres,
      debug_users: allUsers.map(u => `${u.email} (${u.role})`)
    });
  } catch (err) {
...
async function startServer() {
  try {
    ...
    dbInitializationError = err instanceof Error ? err.message : String(err);
    console.error('[SERVER] Critical initialization failure:', dbInitializationError);
  }
